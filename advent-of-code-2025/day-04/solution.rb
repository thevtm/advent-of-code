...
def has_roll(x, y)
  return false if x.negative? || x >= $rows
...
puts "Problem 1 Result: #{$accessible_rolls} ● #{format('%.1f', Time.now - timer_start)}s" # 1351 ● 0.1s
...
loop do
  ...
  (0...$rows).each do |row|
    (0...$cols).each do |col|
      ...
      next unless count_adjacent_rolls(row, col) <= 4
...
puts "Problem 2 Result: #{$removed_rolls_count} ● #{format('%.1f', Time.now - timer_start)}s" # 8345 ● 0.7s
