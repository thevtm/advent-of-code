...
$ingredient_ids = $file_lines[$blank_line_index + 1..].map {|i| i.to_i}
...
$fresh_ingredients = $ingredient_ids.count do |ingredient_id|
  $fresh_ranges.any? {|r| ingredient_id >= r[0] && ingredient_id <= r[1]}
end

puts "Problem 1 Result: #{$fresh_ingredients} ● #{"%.1f" % (Time.now - timer_start)}s" # 733 ● 0.0s
...
puts "Problem 2 Result: #{$fresh_ids_count} ● #{"%.1f" % (Time.now - timer_start)}s" # 345821388687084 ● 0.0s
